Shader "Custom/TextureMask"
{
	Properties
	{
        ...
        _MainMaskReplace("Main Mask Replace Texture", 2D) = "white" {}
        _SecMaskReplace("Secondary Mask Replace Texture", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _MainTex;
            sampler2D _MaskTex;
            sampler2D _MainMaskReplace;
            sampler2D _SecMaskReplace;
            float4 _MaskScale;
			float4 _MaskColor;
            float4 _MainColor;
			half _Blend;
			...
			float4 frag(vertexOutput input) : COLOR
			{
				...
				float4 mainAlbedo = tex2D(_MainMaskReplace, maskPos);
				float4 secAlbedo = tex2D(_SecMaskReplace, maskPos);
				float4 c = lerp(mainAlbedo, secAlbedo, _Blend);

                albedo = (1-isMask)*(albedo+_MainColor) + isMask*c;
